...
class VerifiedCredential(Streamable):
    ...
    def launch(
        ...
        curried_eve_singleton = SerializedProgram.from_program(
            puzzle_for_singleton(
                launcher_coin.name(),
        ...
        launcher_solution = SerializedProgram.to([curried_eve_singleton_hash, uint64(1), None])
    ...
    def do_spend(
        ...
        """
        vc_solution = SerializedProgram.from_program(
            solution_for_singleton(
        ...
        return (
            ...
            CoinSpend(
                self.coin,
                SerializedProgram.from_program(self.construct_puzzle()),
                vc_solution,
            ),
            self._next_vc(
                new_inner_puzzle_hash,
    ...
    def activate_backdoor(
        ...
        return (
            expected_announcement,
            CoinSpend(self.coin, SerializedProgram.from_program(self.construct_puzzle()), vc_solution),
        )
